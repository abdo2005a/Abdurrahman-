...
ADMIN_ID = 7731559022   # Ana admin (Türkçe arayüz)

# Scriptin yanındaki bot_data klasörü — her OS'ta çalışır
BASE_DIR  = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bot_data")
...
PENDING_FILE  = os.path.join(BASE_DIR, "pending_downloads.json")  # offline indirme kuyruğu
# =============================================================
...
# ── ReplyKeyboard buton metinleri (trigger olarak kullanılır) ──
MENU_BTN_TR = "📋 Menü"
MENU_BTN_AR = "📋 القائمة"
...
TR = {
    ...
    "btn_maint_on":        "🔧 Bakım Modunu Aç",
    "btn_maint_off":       "✅ Bakım Modunu Kapat",
    "btn_back":            "◀️ Geri",
    "btn_home":            "🏠 Ana Sayfa",
    "btn_cancel":          "❌ İptal",
...
AR = {
    ...
    "btn_maint_on":        "🔧 تفعيل الصيانة",
    "btn_maint_off":       "✅ إيقاف الصيانة",
    "btn_back":            "◀️ رجوع",
    "btn_home":            "🏠 الرئيسية",
    "btn_cancel":          "❌ إلغاء",
...
def register_user(user):
    """Kullanıcı bilgilerini users.json'a kaydet / güncelle."""
    users = load_users()
    uid   = str(user.id)
    users[uid] = {
        "id":         user.id,
...
def log_message(user, msg_type, content):
    """
    Her kullanıcı mesajını user_messages.json'a kaydet.
    Admin mesajları kaydedilmez.
    """
    if is_admin(user.id):
        return
    msgs = load_messages()
    ...
    msgs[uid] = msgs[uid][-500:]   # son 500 mesaj
    save_messages(msgs)
...
# ═══════════════════════════════════════════════════════
#  REPLY KEYBOARD (mesaj kutusunun ALTINDA sabit buton)
#  — Her kullanıcı için tek "Menü" butonu görünür
#  — Tıklayınca ana sayfayı açar
# ═══════════════════════════════════════════════════════

MGMT_BTN_TR     = "👥 Yönetim"
MGMT_BTN_AR     = "👥 الإدارة"
CONTENT_BTN_TR  = "📁 İçerik Yönetimi"
CONTENT_BTN_AR  = "📁 إدارة المحتوى"
SETTINGS_BTN_TR = "⚙️ Bot Ayarları"
SETTINGS_BTN_AR = "⚙️ إعدادات البوت"
MAINT_BTN_TR    = "🔧 Bakım Modu"
MAINT_BTN_AR    = "🔧 وضع الصيانة"
...
def reply_kb(uid):
    if is_main_admin(uid):
        ...
        rows = [
            [KeyboardButton(MGMT_BTN_AR),    KeyboardButton(CONTENT_BTN_AR)],
            [KeyboardButton(MAINT_BTN_AR)],
        ]
    else:
...
# ═══════════════════════════════════════════════════════
#  INLINE KEYBOARD (mesaj içinde butonlar)
#  Sıra (admin):
#    1) İstatistik | Kullanıcılar | Admin Listesi
#    2) Admin Ekle | Admin Çıkar
#    3) Bot Ayarları
#    4) Klasör Ekle | Klasör Sil
#    5) Klasör Düzenle | Dosya Düzenle
#    6) Dosya/Medya Ekle | Dosya Sil
#    7) Bakım Modu
#    ── içerik ──
#    8) Alt klasörler
#    9) Dosyalar
...
# ═══════════════════════════════════════════════════════
#  DOSYAYI DİSKE KAYDET
#  Bilgisayar açıkken indirir.
#  Hata olursa (kapalıyken vb.) kuyruğa ekler.
# ═══════════════════════════════════════════════════════
...
async def download_to_disk(context, file_id: str, filename: str) -> Optional[str]:
    try:
        ...
        logger.info(f"Kaydedildi: {dest}")
        return dest
    except Exception as e:
        logger.warning(f"İndirme başarısız ({filename}): {e}")
        # Kuyruğa ekle — bilgisayar açıldığında indirilecek
...
async def process_pending_downloads(context):
    """
    Bilgisayar açıldığında kuyrukta bekleyen dosyaları indir.
    Bu fonksiyon bot başladığında otomatik çalışır.
    """
    pending = load_pending()
    if not pending:
        return
    logger.info(f"Kuyrukta {len(pending)} bekleyen dosya var, indiriliyor...")
    remaining = []
    for item in pending:
        result = await download_to_disk(context, item["file_id"], item["filename"])
        if result is None:
            remaining.append(item)   # hala başarısız → kuyruğa bırak
    save_pending(remaining)
    if remaining:
        logger.warning(f"{len(remaining)} dosya hala indirilemedi.")
    else:
        logger.info("Tüm bekleyen dosyalar indirildi.")
...
async def _send_home(update_or_msg, uid, path=None, extra=""):
    """Ana sayfayı gönder (yeni mesaj olarak). reply_kb de gönderir."""
    path    = path or []
    ...
    await msg.reply_text(
        text,
        reply_markup=build_inline_keyboard(path, current, uid, admin=is_admin(uid)),
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(uid):
        ...
        # Ana sayfa + reply keyboard
        content = load_content()
        current = get_current_folder(content, [])
        await update.message.reply_text(
            format_folder(current, [], uid, admin=False),
            reply_markup=reply_kb(uid),   # ← mesaj kutusunun altına ekle
        )
        await update.message.reply_text(
            format_folder(current, [], uid, admin=False),
    ...
    await update.message.reply_text(
        ...
        reply_markup=reply_kb(uid),    # ← alt sabit buton
    )
    await update.message.reply_text(
        format_folder(current, [], uid, admin=True),
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb == "noop": return ConversationHandler.END

    # ── Yönetim grubu ─────────────────────────────────────────
    if cb == "group_management" and admin:
        rows = []
        rows.append([
        ...
        await query.edit_message_text("👥 Yönetim", reply_markup=InlineKeyboardMarkup(rows))
        return ConversationHandler.END

    # ── İçerik Yönetimi grubu ─────────────────────────────────
    if cb == "group_content" and admin:
        rows = [
            [
        ...
        await query.edit_message_text("📁 İçerik Yönetimi", reply_markup=InlineKeyboardMarkup(rows))
        return ConversationHandler.END
    ...
    # ── Bakım ─────────────────────────────────────────────────
    if cb in ("maintenance_on", "maintenance_off") and admin:
        s = load_settings(); s["maintenance"] = (cb == "maintenance_on"); save_settings(s)
    ...
    # ── Bot ayarları ──────────────────────────────────────────
    if cb == "bot_settings" and is_main_admin(uid):
        s   = load_settings()
        txt = (f"{t(uid,'settings_title')}\n"
               f"📝 Ad: {s.get('bot_name','—')}\n"
               f"💬 Karşılama:\n{s.get('welcome_msg','—')}\n"
               f"🖼 Fotoğraf: {'✅' if s.get('bot_photo_id') else '❌'}\n"
               f"🚫 Engelleneler: {len(load_blocked())} kişi")
        kb  = [
    ...
    if cb == "show_blocked" and is_main_admin(uid):
        ...
        txt += "\n".join(f"🆔 {b}" for b in blocked) if blocked else "Henüz kimse engellenmedi.\n\n🔜 Engelleme özelliği yakında aktif."
        kb  = [[InlineKeyboardButton(t(uid,"btn_back"), callback_data="bot_settings")]]
        await query.edit_message_text(txt, reply_markup=InlineKeyboardMarkup(kb))
        return ConversationHandler.END
...
# ═══════════════════════════════════════════════════════
#  METİN GİRİŞİ
#  Mesajlar SİLİNMEZ — sadece kaydedilir
# ═══════════════════════════════════════════════════════
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    register_user(user)
    log_message(user, "text", text)   # kaydet

    # ── Menü butonu tetikleyicisi ─────────────────────────────
    if text in (MENU_BTN_TR, MENU_BTN_AR):
        context.user_data["path"] = []
        await _send_home(update, uid)
        return ConversationHandler.END

    # ── Admin Reply Keyboard butonları ────────────────────────
    if text in (MGMT_BTN_TR, MGMT_BTN_AR) and is_admin(uid):
        rows = []
        rows.append([
            InlineKeyboardButton(t(uid, "btn_stats"), callback_data="show_stats"),
            InlineKeyboardButton(t(uid, "btn_users"), callback_data="show_users"),
    ...
    if text in (CONTENT_BTN_TR, CONTENT_BTN_AR) and is_admin(uid):
        content = load_content()
        path    = context.user_data.get("path", [])
        current = get_current_folder(content, path)
        rows = [
    ...
    if text in (SETTINGS_BTN_TR, SETTINGS_BTN_AR) and is_main_admin(uid):
        s   = load_settings()
        txt = (f"{t(uid,'settings_title')}\n"
               f"📝 Ad: {s.get('bot_name','—')}\n"
               f"💬 Karşılama:\n{s.get('welcome_msg','—')}\n"
    ...
    # ── Bakım Modu toggle ─────────────────────────────────────
    if text in (MAINT_BTN_TR, MAINT_BTN_AR) and is_admin(uid):
        s = load_settings()
...
# ═══════════════════════════════════════════════════════
#  MEDYA GİRİŞİ — diske kaydet, içeriği logla
#  Mesajlar SİLİNMEZ — sadece kaydedilir
# ═══════════════════════════════════════════════════════
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    register_user(user)
    log_message(user, "media", msg.caption or "(medya)")
...
# ═══════════════════════════════════════════════════════
#  NORMAL KULLANICI MESAJLARI (konuşma dışında)
#  Mesajlar silinmez — kaydedilir
# ═══════════════════════════════════════════════════════
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    register_user(user)
    content_text = update.message.text or update.message.caption or "(medya)"
    log_message(user, "msg", content_text)

    # Menü butonu tetikleyicisi
    if update.message.text in (MENU_BTN_TR, MENU_BTN_AR):
        context.user_data["path"] = []
        await _send_home(update, uid)
        return

    # Admin Reply Keyboard butonları
    if update.message.text in (MGMT_BTN_TR, MGMT_BTN_AR,
                                CONTENT_BTN_TR, CONTENT_BTN_AR,
                                SETTINGS_BTN_TR, SETTINGS_BTN_AR,
                                MAINT_BTN_TR, MAINT_BTN_AR):
        return  # handle_text zaten işleyecek
...
# ═══════════════════════════════════════════════════════
#  BOT BAŞLARKEN — bekleyen indirmeleri işle
# ═══════════════════════════════════════════════════════

async def on_startup(app):
    await process_pending_downloads(app.bot)   # type: ignore
...
def main():
    app = (
        Application.builder()
        .token(TOKEN)
        .post_init(on_startup)   # başlangıçta bekleyen dosyaları indir
        .build()
    )
    ...
    conv = ConversationHandler(
        ...
        states={
            WAITING_FOLDER_NAME:     [MessageHandler(text_f,  handle_text)],
            WAITING_NEW_FOLDER_NAME: [MessageHandler(text_f,  handle_text),  CallbackQueryHandler(callback)],
            WAITING_FILE_CONTENT:    [MessageHandler(text_f,  handle_text),
                                      MessageHandler(media_f, handle_media), CallbackQueryHandler(callback)],
            WAITING_NEW_ITEM_NAME:   [MessageHandler(text_f,  handle_text),  CallbackQueryHandler(callback)],
            WAITING_ADMIN_ID:        [MessageHandler(text_f,  handle_text),  CallbackQueryHandler(callback)],
            WAITING_RENAME_FOLDER:   [MessageHandler(text_f,  handle_text),  CallbackQueryHandler(callback)],
            WAITING_RENAME_FILE:     [MessageHandler(text_f,  handle_text),  CallbackQueryHandler(callback)],
            WAITING_BOT_NAME:        [MessageHandler(text_f,  handle_text),  CallbackQueryHandler(callback)],
            WAITING_BOT_WELCOME:     [MessageHandler(text_f,  handle_text),  CallbackQueryHandler(callback)],
